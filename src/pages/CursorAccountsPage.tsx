...
import {
  ...
  getCursorAccountDisplayUserId,
  getCursorAccountLoginProvider,
} from '../types/cursor';
...
const CURSOR_KNOWN_PLAN_FILTERS = ['FREE', 'INDIVIDUAL', 'PRO', 'BUSINESS', 'ENTERPRISE'] as const;
...
export function CursorAccountsPage() {
  ...
  const [isFlowNoticeCollapsed, setIsFlowNoticeCollapsed] = useState<boolean>(() => {
    try {
      return localStorage.getItem(CURSOR_FLOW_NOTICE_COLLAPSED_KEY) === '1';
    } catch {
      return false;
    }
  });
  const [currentAccountId, setCurrentAccountId] = useState<string | null>(() => {
    try {
      const value = localStorage.getItem(CURSOR_CURRENT_ACCOUNT_ID_KEY);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(CURSOR_FLOW_NOTICE_COLLAPSED_KEY, isFlowNoticeCollapsed ? '1' : '0');
    } catch {
  ...
  useEffect(() => {
    try {
      if (currentAccountId) {
        localStorage.setItem(CURSOR_CURRENT_ACCOUNT_ID_KEY, currentAccountId);
      } else {
  ...
  const handleOauthPrepareError = useCallback((e: unknown) => {
    ...
    console.error('[CursorOAuth] 准备授权信息失败', { error: msg });
  ...
  const handleOauthCompleteError = useCallback((e: unknown) => {
    ...
    oauthCompletingRef.current = false;
    oauthActiveRef.current = false;
    oauthLog('Cursor OAuth 授权失败', { loginId: oauthLoginIdRef.current, error: msg });
  }, [oauthLog]);
  ...
  const prepareOauthUrl = useCallback(() => {
    ...
    oauthLog('开始准备 Cursor OAuth 授权信息');
    ...
    cursorService
      ...
      .then((resp) => {
        ...
        oauthActiveRef.current = false;
        return cursorService.completeCursorOAuthLogin(resp.loginId);
      })
      .then(async () => {
        setOauthPolling(false);
        oauthCompletingRef.current = false;
  ...
  useEffect(() => {
    ...
    cursorService.cancelCursorOAuthLogin(loginId).catch(() => {});
    oauthActiveRef.current = false;
  ...
  const handleInjectToVSCode = async (accountId: string) => {
    ...
    try {
      await cursorService.injectCursorToVSCode(accountId);
  ...
  const importFromJsonContent = async (content: string): Promise<number> => {
    const imported = await cursorService.importCursorFromJson(content);
  ...
  const handleImportFromLocalClient = async () => {
    ...
    setAddMessage(t('common.shared.import.importing', '正在导入...'));
    try {
      const imported = await cursorService.importCursorFromLocal();
      await fetchAccounts();
      setAddStatus('success');
      setAddMessage(
        t('common.shared.token.importSuccessMsg', {
  ...
  const handleTokenImport = async () => {
    ...
    try {
      ...
      if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
        const accounts = await cursorService.importCursorFromJson(trimmed);
        importedCount = accounts.length;
      } else {
        await cursorService.addCursorAccountWithToken(trimmed);
        importedCount = 1;
      }
      await fetchAccounts();
      setAddStatus('success');
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const json = await cursorService.exportCursorAccounts(ids);
      const defaultName = `cursor_accounts_${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const creditsSummaryById = useMemo(() => {
    const map = new Map<string, ReturnType<typeof getCursorCreditsSummary>>();
    accounts.forEach((account) => {
      map.set(account.id, getCursorCreditsSummary(account));
    });
    return map;
  }, [accounts]);
  ...
  const resolveCreditsSummary = useCallback(
    (account: (typeof accounts)[number]) =>
      creditsSummaryById.get(account.id) ?? getCursorCreditsSummary(account),
    [creditsSummaryById],
  );
  ...
  const resolvePlanBadgeClass = useCallback((planKey: string) => {
    return getCursorPlanBadgeClass(planKey);
  }, []);
  ...
  const resolvePromptMetrics = useCallback(
    (credits: ReturnType<typeof getCursorCreditsSummary>) =>
  ...
  const resolveDisplayEmail = useCallback((account: (typeof accounts)[number]) => {
    return getCursorAccountDisplayEmail(account).trim();
  }, []);

  const resolveDisplayUserId = useCallback((account: (typeof accounts)[number]) => {
    return getCursorAccountDisplayUserId(account).trim();
  }, []);

  const resolveSignedInWithText = useCallback(
    (account: (typeof accounts)[number]) => {
      const provider = (getCursorAccountLoginProvider(account) || t('cursor.account.providerUnknown', 'Unknown')).trim();
      return t('cursor.account.signedInWith', {
        provider,
  ...
  const resolveBonusExpiryValue = useCallback(
    (credits: ReturnType<typeof getCursorCreditsSummary>) => {
      ...
      if (typeof rawDays !== 'number' || !Number.isFinite(rawDays)) {
        return t('cursor.credits.expiryUnknown', '—');
      }
      return t('cursor.credits.expiryDays', {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((account) => {
        const haystacks = [
          ...
          getCursorAccountDisplayUserId(account),
          getCursorAccountLoginProvider(account) || '',
          account.id,
        ];
    ...
    try {
      await Promise.allSettled(
        affected.map((account) => {
          ...
          return cursorService.updateCursorAccountTags(account.id, nextTags);
        })
      );
      ...
      return (
        ...
        >
          ...
          <div className="account-sub-line">
            <span className="cursor-table-subline" title={`${signedInWithText} | ${t('cursor.account.userId', 'User ID')}: ${maskAccountText(userIdText)}`}>
              {signedInWithText} | {t('cursor.account.userId', 'User ID')}: {maskAccountText(userIdText)}
            </span>
          </div>
          ...
          <div className="ghcp-quota-section">
            <div className="quota-item windsurf-credit-item">
              <div className="quota-header">
                ...
                <span className={`quota-pct ${getCursorQuotaClass(promptMetrics.usedPercent)}`}>
              ...
              <div className="quota-bar-track">
                <div
                  className={`quota-bar ${getCursorQuotaClass(promptMetrics.usedPercent)}`}
            ...
            {showAddOnCredits && (
              <div className="quota-item windsurf-credit-item">
                <div className="quota-header">
                  ...
                  <span className={`quota-pct ${getCursorQuotaClass(addOnMetrics.usedPercent)}`}>
                ...
                <div className="windsurf-credit-meta-row expiry">
                  <span className="windsurf-credit-expiry">
                    {t('cursor.columns.expiry', 'Expiry')}: {bonusExpiryValue}
                  </span>
                </div>
                <div className="quota-bar-track">
                  <div
                    className={`quota-bar ${getCursorQuotaClass(addOnMetrics.usedPercent)}`}
          ...
          <div className="card-footer">
            ...
            <div className="card-actions">
              <button
                ...
                disabled={!!injecting}
                title={t('cursor.injectToVSCode', '切换到 Cursor')}
              >
        ...
        <tr key={groupKey ? `${groupKey}-${account.id}` : account.id} className={isCurrent ? 'current' : ''}>
          ...
          <td>
            <div className="account-cell">
              ...
              <div className="account-sub-line">
                <span className="cursor-table-subline">
                  {signedInWithText} | {t('cursor.account.userId', 'User ID')}: {maskAccountText(userIdText)}
                </span>
              </div>
          ...
          <td>
            <div className="quota-item windsurf-table-credit-item">
              <div className="quota-header">
                ...
                <span className={`quota-value ${getCursorQuotaClass(promptMetrics.usedPercent)}`}>
                  {promptMetrics.usedPercent}%
                </span>
              </div>
              ...
              <div className="quota-progress-track">
                <div
                  className={`quota-progress-bar ${getCursorQuotaClass(promptMetrics.usedPercent)}`}
                  style={{ width: `${promptMetrics.usedPercent}%` }}
                />
              </div>
            </div>
            ...
            {showAddOnCredits ? (
              <div className="quota-item windsurf-table-credit-item">
                <div className="quota-header">
                  ...
                  <span className={`quota-value ${getCursorQuotaClass(addOnMetrics.usedPercent)}`}>
                    {addOnMetrics.usedPercent}%
                  </span>
                </div>
                <div className="windsurf-credit-meta-row table">
                ...
                <div className="quota-progress-track">
                  <div
                    className={`quota-progress-bar ${getCursorQuotaClass(addOnMetrics.usedPercent)}`}
                    style={{ width: `${addOnMetrics.usedPercent}%` }}
                  />
                </div>
              </div>
            ) : (
              <div className="quota-empty">{t('cursor.credits.expiryUnknown', '—')}</div>
            )}
          </td>
            ...
            <ul className="ghcp-flow-notice-list">
              ...
              <li>
                {t(
                  ...
                  'Data scope: only Cursor auth-session related entries are read/updated; system secrets are not modified and no key/token is uploaded.',
                )}
              </li>
            </ul>
          </div>
          ...
          <div className="modal-content ghcp-add-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              {addTab === 'oauth' && (
                <div className="add-section">
                  <p className="section-desc">
                    {t('cursor.oauth.desc', '点击下方按钮，在浏览器中完成 Cursor 授权登录。')}
                  </p>
              ...
              {addTab === 'token' && (
                <div className="add-section">
                  <p className="section-desc">
                    {t('cursor.token.desc', '粘贴您的 Cursor Access Token 或导出的 JSON 数据。')}
                  </p>
                  <textarea
              ...
              {addTab === 'import' && (
                <div className="add-section">
                  <p className="section-desc">
                    {t('cursor.import.localDesc', '支持从本机 Cursor 客户端或 JSON 文件导入账号数据。')}
                  </p>
