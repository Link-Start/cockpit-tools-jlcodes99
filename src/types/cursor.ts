...
export interface CursorAccount {
  ...
  cursor_auth_token_raw?: unknown;
  cursor_profile_raw?: unknown;
...
export type CursorQuotaClass = 'high' | 'medium' | 'low' | 'critical';
export type CursorPlanBadge = 'FREE' | 'INDIVIDUAL' | 'PRO' | 'BUSINESS' | 'ENTERPRISE' | 'UNKNOWN';
...
export type CursorCreditsSummary = {
...
function resolveRawEmail(account: CursorAccount): string | null {
  const fromUsage = firstString(account.cursor_usage_raw, [
    ['userInfo', 'email'],
    ['email'],
  ]);
  if (isLikelyEmail(fromUsage)) return fromUsage;

  const fromProfile = firstString(account.cursor_profile_raw, [
    ['email'],
    ['user', 'email'],
    ['account', 'email'],
    ['primaryEmail'],
  ...
  const fromAuth = firstString(account.cursor_auth_token_raw, [
    ['email'],
    ['userEmail'],
    ['login_hint'],
    ['loginHint'],
...
function resolveRawUserId(account: CursorAccount): string | null {
  const userId = account.user_id?.trim();
  if (userId && !isPlaceholderIdentity(userId)) return userId;

  return (
    firstString(account.cursor_usage_raw, [
      ['userInfo', 'userId'],
      ['userId'],
      ['user_id'],
    ]) ||
    firstString(account.cursor_profile_raw, [
      ['userId'],
      ['user_id'],
      ['id'],
      ['sub'],
    ...
    firstString(account.cursor_auth_token_raw, [
      ['userId'],
      ['user_id'],
      ['sub'],
      ['accountId'],
...
function resolveRawProvider(account: CursorAccount): string | null {
...
function resolvePlanName(account: CursorAccount): string | null {
  return (
    ...
    firstString(account.cursor_usage_raw, [
      ...
      ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'displayName'],
      ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'displayNamePlural'],
      ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'type'],
...
function getUsageRoot(account: CursorAccount): unknown {
  return (
    getPathValue(account.cursor_usage_raw, ['cursor.resourceNotifications.usageState']) ||
    getPathValue(account.cursor_usage_raw, ['usageState']) ||
    account.cursor_usage_raw
  );
}

function getUsageBreakdowns(account: CursorAccount): Record<string, unknown>[] {
...
function collectActiveBonusUsages(account: CursorAccount): ParsedBonusUsage[] {
...
function resolvePromptTotal(account: CursorAccount): number | null {
  ...
  return (
    firstNumber(getUsageRoot(account), [
      ...
      ['credits', 'total'],
      ['totalCredits'],
      ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'usageLimit'],
      ['usageState', 'usageBreakdowns', '0', 'usageLimit'],
    ])
  );
}

function resolvePromptUsed(account: CursorAccount): number | null {
  ...
  return (
    firstNumber(getUsageRoot(account), [
      ...
      ['credits', 'used'],
      ['usedCredits'],
      ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'currentUsage'],
      ['usageState', 'usageBreakdowns', '0', 'currentUsage'],
    ])
  );
}

function resolveAddOnTotal(account: CursorAccount): number | null {
  ...
  return (
    firstNumber(getUsageRoot(account), [
      ...
      ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'freeTrialUsage', 'usageLimit'],
      ['usageState', 'usageBreakdowns', '0', 'freeTrialUsage', 'usageLimit'],
    ])
  );
}

function resolveAddOnUsed(account: CursorAccount): number | null {
  ...
  return (
    firstNumber(getUsageRoot(account), [
      ...
      ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'freeTrialUsage', 'currentUsage'],
      ['usageState', 'usageBreakdowns', '0', 'freeTrialUsage', 'currentUsage'],
    ])
  );
}

function resolvePlanEnd(account: CursorAccount): number | null {
  return (
    ...
    firstTimestamp(getUsageRoot(account), [
      ...
      ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'resetDate'],
      ['usageState', 'usageBreakdowns', '0', 'resetDate'],
    ])
  );
}

function resolveBonusExpireDays(account: CursorAccount): number | null {
  ...
  const usageDays = firstNumber(getUsageRoot(account), [
    ...
    ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'freeTrialUsage', 'daysRemaining'],
    ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'freeTrialUsage', 'expiryDays'],
  ...
  const expiryTs = firstTimestamp(getUsageRoot(account), [
    ...
    ['bonus', 'expiryDate'],
    ['cursor.resourceNotifications.usageState', 'usageBreakdowns', '0', 'freeTrialUsage', 'expiryDate'],
...
export function getCursorPlanDisplayName(planType?: string | null): string {
...
export function getCursorPlanBadgeClass(planType?: string | null): string {
  const plan = getCursorPlanDisplayName(planType);
...
export function getCursorPlanBadge(account: CursorAccount): CursorPlanBadge {
  const plan = getCursorPlanDisplayName(resolvePlanName(account));
  if (
    plan === 'FREE' ||
    plan === 'PRO' ||
    plan === 'INDIVIDUAL' ||
...
export function getCursorQuotaClass(percentage: number): CursorQuotaClass {
...
export function getCursorAccountDisplayUserId(account: CursorAccount): string {
  const userId = resolveRawUserId(account);
  if (userId && !isPlaceholderIdentity(userId)) return userId;
...
export function getCursorAccountLoginProvider(account: CursorAccount): string | null {
...
export function getCursorCreditsSummary(account: CursorAccount): CursorCreditsSummary {
...
export function getCursorUsage(account: CursorAccount): CursorUsage {
  const summary = getCursorCreditsSummary(account);
...
export function formatCursorResetTime(resetAt: number | null | undefined, t: Translate): string {
  ...
  const diff = ts - now;
  if (diff <= 0) {
    return t('cursor.credits.resetNow', {
      defaultValue: '即将重置',
    });
  }
  ...
  if (hours > 0) {
    return t('cursor.credits.resetInHours', {
      hours,
      defaultValue: '{{hours}} 小时后重置',
    });
  }
  const minutes = Math.max(1, Math.floor(diff / 60));
  return t('cursor.credits.resetInMinutes', {
    minutes,
