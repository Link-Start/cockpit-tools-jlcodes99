...
pub async fn cursor_open_instance_window(instance_id: String) -> Result<(), String> {
    ...
    if let Err(err) = modules::cursor_instance::focus_cursor_instance(
        instance.last_pid,
        Some(&instance.user_data_dir),
    ) {
        modules::logger::log_warn(&format!(
            "定位 Cursor 实例窗口失败，回退为启动实例: instance_id={}, err={}",
            instance.id, err
        ));
        let extra_args = modules::process::parse_extra_args(&instance.extra_args);
        let pid = modules::cursor_instance::start_cursor_with_args_with_new_window(
        ...
        let _ = modules::cursor_instance::update_instance_after_start(&instance.id, pid)?;
    }
