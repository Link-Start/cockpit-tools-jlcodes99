...
use crate::models::cursor::{CursorAccount, CursorAccountIndex, CursorOAuthCompletePayload};
use crate::modules::{account, cursor_oauth, logger};

const ACCOUNTS_INDEX_FILE: &str = "cursor_accounts.json";
const ACCOUNTS_DIR: &str = "cursor_accounts";
const LOCAL_AUTH_ACCESS_TOKEN_KEY: &str = "cursorAuth/accessToken";
const LOCAL_AUTH_REFRESH_TOKEN_KEY: &str = "cursorAuth/refreshToken";
const LOCAL_AUTH_CACHED_EMAIL_KEY: &str = "cursorAuth/cachedEmail";
const LOCAL_AUTH_PLAN_KEY: &str = "cursorAuth/stripeMembershipType";
const LOCAL_AUTH_FALLBACK_ACCESS_TOKEN_KEY: &str = "cursor.accessToken";
const LOCAL_AUTH_FALLBACK_EMAIL_KEY: &str = "cursor.email";
const LOCAL_USAGE_DB_KEYS: [&str; 6] = [
    "cursor.resourceNotifications.usageState",
    "cursor.usageState",
    "cursor.usage_state",
    "cursorUsageState",
    "cursorUsage",
    "cursor.resourceNotifications",
];
...
fn payload_profile_arn(payload: &CursorOAuthCompletePayload) -> Option<String> {
    pick_profile_arn(payload.cursor_profile_raw.as_ref())
        .or_else(|| pick_profile_arn(payload.cursor_auth_token_raw.as_ref()))
}

fn account_profile_arn(account: &CursorAccount) -> Option<String> {
    pick_profile_arn(account.cursor_profile_raw.as_ref())
        .or_else(|| pick_profile_arn(account.cursor_auth_token_raw.as_ref()))
}
...
fn merge_duplicate_account(primary: &mut CursorAccount, duplicate: &CursorAccount) {
    ...
    fill_if_none(
        &mut primary.cursor_auth_token_raw,
        &duplicate.cursor_auth_token_raw,
    );
    fill_if_none(&mut primary.cursor_profile_raw, &duplicate.cursor_profile_raw);
...
fn normalize_account_index(index: &mut CursorAccountIndex) -> Vec<CursorAccount> {
    ...
    let preferred_bound_id = crate::modules::cursor_instance::load_default_settings()
        .ok()
...
fn apply_payload(account: &mut CursorAccount, payload: CursorOAuthCompletePayload) {
    ...
    account.cursor_auth_token_raw = payload.cursor_auth_token_raw;
    account.cursor_profile_raw = payload.cursor_profile_raw;
    account.cursor_usage_raw = payload.cursor_usage_raw;
    account.last_used = now_ts();
}

pub fn upsert_account(payload: CursorOAuthCompletePayload) -> Result<CursorAccount, String> {
    let _lock = CURSOR_ACCOUNT_INDEX_LOCK
        .lock()
        .map_err(|_| "获取 Cursor 账号锁失败".to_string())?;
    let now = now_ts();
    ...
    let mut account = existing.unwrap_or(CursorAccount {
        ...
        cursor_auth_token_raw: payload.cursor_auth_token_raw.clone(),
        cursor_profile_raw: payload.cursor_profile_raw.clone(),
...
pub async fn refresh_account_token(account_id: &str) -> Result<CursorAccount, String> {
    ...
    let payload = cursor_oauth::refresh_payload_for_account(&account).await?;
    ...
    logger::log_info(&format!(
        "[Cursor Refresh] 刷新完成: id={}, email={}, elapsed={}ms",
        updated.id,
        updated.email,
...
fn read_state_db_value(conn: &Connection, key: &str) -> Result<Option<String>, String> {
    conn.query_row("SELECT value FROM ItemTable WHERE key = ?1", [key], |row| {
        row.get::<_, String>(0)
    })
    .optional()
    .map_err(|e| format!("读取 Cursor 本地键失败({}): {}", key, e))
}

pub fn read_local_auth_token_json() -> Result<Option<Value>, String> {
    let path = get_default_cursor_state_db_path()?;
    if !path.exists() {
        return Ok(None);
    }

    let conn = Connection::open(&path)
        .map_err(|e| format!("打开 Cursor 本地数据库失败({}): {}", path.display(), e))?;

    let access_token = read_state_db_value(&conn, LOCAL_AUTH_ACCESS_TOKEN_KEY)?
        .or_else(|| read_state_db_value(&conn, LOCAL_AUTH_FALLBACK_ACCESS_TOKEN_KEY).ok().flatten());
    if access_token.as_deref().unwrap_or("").trim().is_empty() {
        return Ok(None);
    }

    let refresh_token = read_state_db_value(&conn, LOCAL_AUTH_REFRESH_TOKEN_KEY)?;
    let cached_email = read_state_db_value(&conn, LOCAL_AUTH_CACHED_EMAIL_KEY)?
        .or_else(|| read_state_db_value(&conn, LOCAL_AUTH_FALLBACK_EMAIL_KEY).ok().flatten());
    let plan_name = read_state_db_value(&conn, LOCAL_AUTH_PLAN_KEY)?;
    ...
    if let Some(value) = refresh_token.and_then(|value| {
        let trimmed = value.trim();
        if trimmed.is_empty() {
            None
        } else {
        ...
        obj.insert("refreshToken".to_string(), Value::String(value));
    }
    if let Some(value) = cached_email.and_then(|value| {
        let trimmed = value.trim();
        if trimmed.is_empty() {
            None
        } else {
        ...
        obj.insert("cachedEmail".to_string(), Value::String(value));
    }
    if let Some(value) = plan_name.and_then(|value| {
        let trimmed = value.trim();
        if trimmed.is_empty() {
            None
        } else {
        ...
        obj.insert("stripeMembershipType".to_string(), Value::String(value));
    }
    obj.insert("tokenType".to_string(), Value::String("Bearer".to_string()));
...
pub fn read_local_profile_json() -> Result<Option<Value>, String> {
    ...
    let conn = Connection::open(&path)
        .map_err(|e| format!("打开 Cursor 本地数据库失败({}): {}", path.display(), e))?;
    let email = read_state_db_value(&conn, LOCAL_AUTH_CACHED_EMAIL_KEY)?
        .or_else(|| read_state_db_value(&conn, LOCAL_AUTH_FALLBACK_EMAIL_KEY).ok().flatten());
...
pub fn read_local_usage_snapshot() -> Result<Option<Value>, String> {
    ...
    for key in LOCAL_USAGE_DB_KEYS {
        let raw = read_state_db_value(&conn, key)?;
